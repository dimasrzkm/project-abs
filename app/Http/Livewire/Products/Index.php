...
class Index extends Component
{
    ...
    public $editedProductId = null;

    public $deletedProductIndex = null;
    ...
    public function resetAttributes()
    {
        ...
        if ($sumOfRecipes <= 1) {
            ...
            $this->addNewRecipe();
        } elseif ($sumOfRecipes > 1 && ! is_null($this->recipeStocks)) {
    ...
    public function updateProduct()
    {
        ...
        if (! is_null($product)) {
            ...
            } elseif (count($this->recipeStocks) != $oldTotalIngredients) {  // kondisi dimana perubahan ada pada recipes
                for ($i = 0; $i < count($this->recipeStocks); $i++) {
                    $editedProduct->stocks()->syncWithoutDetaching([$this->recipeStocks[$i]['stock_id'] => ['total' => $this->recipeStocks[$i]['amount']]]);
                    Stock::find($this->recipeStocks[$i]['stock_id'])->decrement('amount', ((int) $this->stock - $oldStock) * $this->recipeStocks[$i]['amount']);
                }
